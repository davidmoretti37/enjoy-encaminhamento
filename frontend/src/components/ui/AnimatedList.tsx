...
function AnimatedList<T>({
  ...
  const defaultRenderItem = (item: T, index: number, isSelected: boolean) => (
    <div className={`animated-list-item ${isSelected ? 'selected' : ''} ${itemClassName}`}>
  ...
  return (
    <div className={`animated-list-container ${className}`}>
      ...
      {showGradients && (
        <>
          <div className="animated-list-gradient-top" style={{ opacity: topGradientOpacity }}></div>
          <div className="animated-list-gradient-bottom" style={{ opacity: bottomGradientOpacity }}></div>
        </>
      )}
    </div>
  );
