...
const toolsList = [
  { id: 'searchWeb', name: 'Buscar na web', shortName: 'Buscar', icon: GlobeIcon },
  { id: 'writeCode', name: 'Escrever ou codificar', shortName: 'Escrever', icon: PencilIcon },
  { id: 'deepResearch', name: 'Pesquisa profunda', shortName: 'Pesquisa', icon: TelescopeIcon },
  { id: 'thinkLonger', name: 'Pensar mais', shortName: 'Pensar', icon: LightbulbIcon },
];

// --- The PromptBox Component ---
export const AIPromptBox = React.forwardRef<HTMLTextAreaElement, React.TextareaHTMLAttributes<HTMLTextAreaElement> & { onSend?: (message: string) => void; disabled?: boolean }>(
  ({ className, onSend, disabled, ...props }, ref) => {
    ...
    return (
      <div className={cn("flex flex-col rounded-[28px] p-2 shadow-lg transition-colors bg-white border border-gray-200", className)}>
        ...
        {imagePreview && (
          <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
            <div className="relative mb-1 w-fit rounded-[1rem] px-1 pt-1">
              ...
              <button onClick={handleRemoveImage} className="absolute right-2 top-2 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-white/80 text-black transition-colors hover:bg-gray-200" aria-label="Remove image">
        ...
        <textarea
          ...
          disabled={disabled}
          placeholder="Pergunte sobre candidatos, empresas, contratos..."
          className="custom-scrollbar w-full resize-none border-0 bg-transparent p-3 text-gray-900 placeholder:text-gray-400 focus:ring-0 focus-visible:outline-none min-h-16 disabled:opacity-50"
          {...props}
        />
            ...
            <div className="flex items-center gap-2">
              <Tooltip>
                <TooltipTrigger asChild>
                  <button type="button" onClick={handlePlusClick} className="flex h-8 w-8 items-center justify-center rounded-full text-gray-600 transition-colors hover:bg-gray-100 focus-visible:outline-none">
                    <PlusIcon className="h-6 w-6" />
                    <span className="sr-only">Anexar imagem</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="top" showArrow={true}><p>Anexar imagem</p></TooltipContent>
              </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <PopoverTrigger asChild>
                      <button type="button" className="flex h-8 items-center gap-2 rounded-full p-2 text-sm text-gray-600 transition-colors hover:bg-gray-100 focus-visible:outline-none">
              ...
              {activeTool && (
                <>
                  ...
                  <button
                    ...
                    className="flex h-8 items-center gap-2 rounded-full px-2 text-sm hover:bg-gray-100 cursor-pointer text-purple-600 transition-colors"
                  >
              ...
              <div className="ml-auto flex items-center gap-2">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full text-gray-600 transition-colors hover:bg-gray-100 focus-visible:outline-none">
                      <MicIcon className="h-5 w-5" />
                      <span className="sr-only">Gravar voz</span>
                    </button>
                  </TooltipTrigger>
                  <TooltipContent side="top" showArrow={true}><p>Gravar voz</p></TooltipContent>
                </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none bg-gray-900 text-white hover:bg-gray-800 disabled:bg-gray-300"
                    >
