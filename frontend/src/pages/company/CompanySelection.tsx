...
export default function CompanySelection() {
  ...
  // New batch queries
  const { data: lockedBatches, isLoading: lockedLoading } = trpc.batch.getLockedBatches.useQuery(
    undefined,
    { enabled: !!user && user.role === 'company' }
  );
  ...
  const payForBatchMutation = trpc.batch.payForBatch.useMutation({
    onSuccess: (data) => {
      toast.success('Pagamento iniciado! Use a página de pagamentos para confirmar.');
      setLocation('/company/payments');
    },
    onError: (error) => {
    ...
    <DashboardLayout>
      <div className="space-y-8">
        ...
        {isLoading ? (
          ...
          <>
            {/* LOCKED BATCHES - Pay to Unlock */}
            {lockedBatches && lockedBatches.length > 0 && (
              <Card className="border-amber-300 bg-amber-50">
                <CardHeader>
                  <div className="flex items-center gap-2">
                    <Lock className="h-5 w-5 text-amber-700" />
                    <CardTitle className="text-amber-900">
                      Candidatos Disponíveis - Pague para Desbloquear
                    </CardTitle>
                  </div>
                  <CardDescription className="text-amber-700">
                    Novos candidatos foram encontrados. Pague para visualizar os detalhes.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {lockedBatches.map((batch: any) => (
                    <Card key={batch.id} className="bg-white border-amber-200">
                      <CardContent className="pt-6">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <h3 className="font-semibold text-lg">{batch.job?.title}</h3>
                            <p className="text-sm text-gray-600 mt-1">
                              {batch.candidate_count} candidatos qualificados encontrados
                            </p>
                          </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">Taxa de desbloqueio</p>
                          </div>
                          <Button
                            onClick={() => payForBatchMutation.mutate({
                              batchId: batch.id,
                              paymentMethod: 'pix'
                            })}
                            size="lg"
                            disabled={payForBatchMutation.isPending}
                          >
                            <DollarSign className="h-4 w-4 mr-2" />
                            {payForBatchMutation.isPending ? 'Processando...' : 'Pagar e Desbloquear'}
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
                ...
                {unlockedBatches.map((batch: any) => (
                  <Card key={batch.id}>
                    <CardHeader>
                      ...
                      <CardDescription>
                        {batch.candidates?.length || 0} candidatos disponíveis
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
